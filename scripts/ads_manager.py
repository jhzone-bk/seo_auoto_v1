...
class JDStrategy(BaseAdStrategy):
    ...
    def get_html(self, keyword):
        """获取京东广告 HTML，调用 API 失败则使用 fallback"""
        try:
        ...
        # API 未返回有效链接，使用 fallback（不触发熔断，这只是无数据不是异常）
...
class AdsManager:
    ...
    def get_all_ads(self, keyword):
        ...
        if not ads:
            ads.append(
                ...
                '<a href="https://www.jd.com/" rel="nofollow sponsored" data-ad-source="hard_fallback">立即选购 &raquo;</a></p>'
                '</div>'
            )
    ...
    def inject_into_html(self, html_content, keyword):
        ...
        for i, ad_html in enumerate(ads):
            ...
            # 重新计算在当前 result 中的位置（因为之前的插入已改变长度）
            # 简化处理：直接在原始 html_content 上一次性从后往前插入
            pass
