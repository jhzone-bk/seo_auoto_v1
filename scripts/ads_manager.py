...
class BaseAdStrategy(ABC):
    ...
    def __init__(self, name, config):
        ...
        self._last_failure = 0          # 上次失败时间�?
        self._circuit_open = False      # 熔断器状�?
    ...
    def get_link(self, keyword):
        ...
        返回: (success: bool, url: str) �?(False, None)
        """
        pass
    ...
    def get_html(self, keyword):
        ...
        返回: str �?完整的广�?HTML，失败时返回 fallback HTML
        """
        pass
...
class JDStrategy(BaseAdStrategy):
    ...
    def get_html(self, keyword):
        """获取京东广告 HTML，调�?API 失败则使�?direct_link"""
        try:
        ...
        # API �?direct_link 都不可用，使�?fallback（不触发熔断，这只是无数据不是异常）
...
class TaobaoStrategy(BaseAdStrategy):
    ...
    def get_html(self, keyword):
        """获取淘宝广告 HTML，调�?API 失败则使�?fallback"""
        try:
            success, link = self.get_link(keyword)
        except Exception:
            self._trip_circuit()
        ...
        if success and link:
            return (
                ...
                f'<a href="{link}" rel="nofollow sponsored"{tracking}>去淘宝逛�?&raquo;</a></p>'
                '</div>'
            )
...
class LeadStrategy(BaseAdStrategy):
    ...
    def get_html(self, keyword):
        """获取线索转化表单 HTML，调�?API 失败则使�?fallback"""
        try:
            success, link = self.get_link(keyword)
        except Exception:
            self._trip_circuit()
...
class AdsManager:
    ...
    def _init_strategies(self):
        ...
        print(f"  [ADS] 已加�?{len(loaded)} 条策�? {[s.name for s in loaded]}")
        return loaded
    ...
    def _match_keyword_ads(self, keyword):
        ...
        for match_key, ad_data in self.keyword_ad_mapping.items():
            # substring 匹配：关键词包含 match_key 即命�?
    ...
    def get_all_ads(self, keyword):
        ...
        流程:
            1. 精准匹配 (keyword_ad_mapping) �?优先占据�?个广告位
            2. 策略广告 (JD/淘宝/线索) �?填充剩余广告�?
            3. 通用广告�?(default_ad.pool) �?轮询兜底
            4. 硬编码兜�?�?绝对不返回空列表
        ...
        返回:
            list[str]: 广告 HTML 片段列表（最�?max_ads 个，最�?1 个）
        """
        ads = []
        ...
        if not ads:
            ads.append(
                ...
                '<strong style="font-size:16px;color:#333;">[广告] 汽车用品精�?/strong>'
    ...
    def inject_into_html(self, html_content, keyword):
        ...
        使用 config 中配置的插入位置（默�?after_p2, after_p4, after_p6），
        即第 2/4/6 �?</p> 之后，模拟首屏→中屏→文末的广告分布�?
        """
        import re
        ...
        # 插入位置映射：第 2/4/6 �?</p>�?-indexed: 1, 3, 5�?
        ...
        for i, ad_html in enumerate(ads):
            ...
            # 重新计算在当�?result 中的位置（因为之前的插入已改变长度）
            # 简化处理：直接在原�?html_content 上一次性从后往前插�?
            pass
...
def create_ads_manager():
    """创建 AdsManager 实例，自动加�?config.json"""
    return AdsManager()